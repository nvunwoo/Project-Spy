...
| 배포 대상 | GitHub Pages 정적 훈련판. 게시·접속 검증 결과는 정리 완료 뒤 갱신 |
| 실전 멀티플레이 | 서버 권위 API, 영속 저장, 2인 연결·동기화, 비밀 정보의 서버 보관은 미구현 |
| 외부 서비스 | Vercel 팀에 Project-Spy 프로젝트 없음 확인. Supabase Project-Spy 비활성 프로젝트 삭제 작업 대상 |
...
- [ ] 로컬 변경과 문서·코드·Excel·참고 자료를 main에 업로드하고 원격 파일·커밋 확인
- [ ] main 이외의 로컬·원격 브랜치 0개 확인
- [ ] GitHub Pages 실제 URL과 desktop·태블릿 크기 플레이 진입 확인
- [x] Vercel 연결 팀에서 Project-Spy 프로젝트가 없음을 읽기 전용으로 확인
- [ ] Supabase Project-Spy 삭제 및 목록에서 제거 확인
- [ ] 업로드할 비밀값이 없음을 최종 확인
