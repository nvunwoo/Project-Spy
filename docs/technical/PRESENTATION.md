...
> 기술 기준 버전: **t0.2**<br>
> 게임 규칙 기준: **v0.6**<br>
> 구현 상태: **APPROVED / PHASE 0B STARTED / BASIC-PRIMITIVES FOUNDATION LOCAL VERIFIED**<br>
...
> UI 기준: 로비, HUD, 명령 작성, 반응형과 접근성은 [UI_UX_CONTRACT.md](./UI_UX_CONTRACT.md)를 따른다.
...
2026-08-22 사용자 승인 뒤 Next.js·React Three Fiber 앱, 전체 화면 기본 도형 지도, 논리 좌표 adapter와 `SelectionIntent`, 반투명 HUD와 pan·zoom 중 HUD 투명화를 구현하고 현재 fixture 한 턴 범위를 로컬 검증했다. 최종 3D 에셋, 권위 있는 서버 이동 replay와 실기기 성능 검증은 아직 만들거나 수행하지 않았다.
...
- 플레이 화면에는 제작용 `도로`, `A/B/C`, `K/H/D`, 고정/충원 스폰 표기를 그대로 노출하지 않는다.
...
4. 서버 game status가 `waiting` 또는 `setup`일 때 capability·Canvas 초기화가 실패하면 WebGL1로 묵시적 강등하거나 지원 완료를 주장하지 않는다. 호환성 안내, Chrome·그래픽 가속 확인, 다시 시도와 허용된 작전 취소가 있는 DOM 화면을 표시한다.
5. 서버 status가 `active`이면 페이지 수명이 아니라 경기 수명을 우선한다. 플레이 중 `webglcontextlost`뿐 아니라 새로고침·직접 URL·복구 목록 재진입의 첫 capability check, Canvas 초기화와 dynamic scene load가 실패해도 DOM HUD를 유지하고 context 복구와 최신 projection 재렌더를 시도한다.
6. active 경기의 복구가 실패하면 단순 2D 논리 격자와 동등한 요원·도달 셀·행동 DOM 목록을 같은 logical ID·명령 API에 연결해 경기를 계속한다. 이는 최초부터 WebGL2가 없는 기기를 정식 지원한다는 뜻이 아니라 활성 경기의 중단 방지 계약이다.

Three.js의 현재 `WebGLRenderer`는 WebGL2를 사용하며 WebGL1은 지원하지 않는다. capability check와 오류 메시지는 [Three.js WebGL 유틸리티](https://threejs.org/docs/pages/WebGL.html)를 기준으로 한다. WebGPU는 t0.2 필수 범위가 아니다.

## 5. 서버 승인 경로와 이동 연출

클라이언트가 권위 있는 이동 결과를 계산하거나 추측하지 않는다. 명령 작성 중에는 공개 맵 projection만으로 대표 최단 합법 경로를 제안할 수 있지만, 이는 사용자가 수정·확정하는 미리보기이며 서버가 canonical state로 다시 검증한다. 실행 연출은 서버가 해당 플레이어에게 공개해도 되는 이동 결과에 논리 waypoint 목록을 포함했을 때만 재생한다.
...
t0.2에서는 다음을 사용하지 않는다.

- 물리 엔진과 rigid-body 충돌
- NavMesh 또는 숨은 상태를 사용하는 권위 클라이언트 pathfinding
- 메시 경계를 이용한 이동 가능 판정
...
- t0.2 정식 지원 브라우저는 Android용 Google Chrome이다.
...
- 첫 구현은 **북쪽 고정 3/4 perspective 카메라**를 사용한다. RED/BLUE에 따라 논리 방향을 뒤집지 않으며 자유 회전을 제공하지 않는다.
- 단일 탭·클릭·pen tap은 요원 또는 셀 선택이다. 드래그 임계값은 실제 기기에서 8~12 CSS px 범위로 조정한다.
- 태블릿은 두 손가락 이동으로 제한된 pan, pinch로 제한된 zoom을 사용한다. 한 손가락 drag를 필수 pan으로 쓰지 않아 셀 선택과 충돌시키지 않는다.
- 마우스는 wheel zoom과 오른쪽 또는 가운데 버튼 drag pan을 제공한다. 키보드와 DOM `시점 초기화` 버튼으로 같은 복구 동작을 제공한다.
- 요원 카드 선택 시 짧은 focus 이동을 허용하지만, 자동 이동 중 사용자의 입력을 오래 잠그지 않는다. `prefers-reduced-motion`에서는 즉시 또는 매우 짧게 스냅한다.
- 게임은 Galaxy Tab S9+와 긴 화면을 위한 **가로 전용**이다. `width <= height`에서는 Canvas·HUD·명령 입력을 차단하는 orientation gate를 표시하며 CSS로 화면을 90도 돌리거나 portrait gameplay를 재배치하지 않는다.
- 게임 셸은 `100dvh`, `env(safe-area-inset-*)`, `visualViewport`와 resize를 반영한다. 닉네임 IME, 브라우저 툴바, 분할 화면과 방향 전환 뒤 가로일 때만 Canvas·카메라·hit map을 다시 계산하고, 가로 복귀 시 기존 화면 상태를 유지한다.
...
- resize, 분할 화면, 가상 키보드와 방향 전환 뒤 가로 viewport에서는 Canvas 크기와 카메라 투영을 다시 계산하고 세로에서는 orientation gate로 전환한다.
- 사용자가 품질을 직접 선택하는 UI는 설정 드로어에 두고 `자동/Balanced/Low/High`와 현재 적용값을 텍스트로 표시한다. 개발용 강제 프리셋과 측정 HUD는 production UI와 분리한다.

## 9. 성능·호환성 수용 기준

### 9.1 Android 태블릿 지원 정책

2026-08-18 기준 Project-Spy의 Android 정책은 다음과 같다. Chrome의 공식 OS 하한은 현재 Android 10이지만, 이는 Chrome 설치 가능 조건일 뿐 Project-Spy의 장기 품질 보증선과 같지 않다. 공식 하한과 최신 버전은 변할 수 있으므로 실기기 검증과 배포 직전에 [Chrome 시스템 요구사항](https://support.google.com/chrome/a/answer/7100626?hl=en)과 [Chrome 릴리스 채널](https://developer.chrome.com/docs/web-platform/chrome-release-channels/)을 다시 확인한다.
...
초기 데모의 공통 측정 장면은 전체 16×16 맵, 100개 건물 부지, 공항·항구의 4칸 표현, 요원 6명, 선택 표식과 6명의 동시 waypoint 이동을 포함한다. 개발 빌드가 아니라 최적화된 production build에서 로드 완료 후 60초 이상 측정한다.
...
사용자는 목표 기기가 **Galaxy Tab S9+**임을 확인했다. Samsung 공개 사양의 12.4인치·2800×1752·12GB 구성은 대표 기기 계획에 참고하되 CSS breakpoint에는 사용하지 않는다. 정확한 모델 코드, 실제 RAM, Android·Chrome 버전과 landscape CSS viewport는 첫 실기기 검증 전에 장치에서 직접 기록한다. 사양 근거는 [Samsung Galaxy Tab S9+ 제품 사양](https://www.samsung.com/ie/tablets/galaxy-tab-s/galaxy-tab-s9-plus-wi-fi-beige-256gb-sm-x810nzeaeub/)이다.
...
| 화면   | 1.0/1.5/2.0 DPR, landscape resize·가상 키보드, portrait orientation gate, 가로 복귀 상태 보존                                          |
| 좌표   | 네 모서리와 중심 셀, 역변환, 공항·항구 네 내부 셀, 건물 도착, 도로 통과                                                                |
| 이동   | 승인/거절, 오래된 버전 취소, 재접속 스냅, 6명 동시 이동, 숨은 상대 경로 미노출                                                         |
...
| 접근성 | hover 없이 전 기능 수행, 48×48 CSS px 터치 대상, DOM 대체 목록, DOM 닉네임 IME, 오류 텍스트, 포커스 순서, 200% 텍스트와 reduced motion |

### 9.4 현재 기본 도형 fixture 검증 결과 — 2026-08-22

- 논리 map 좌표와 R3F world 좌표의 adapter, 기본 도형 도시 장면과 `SelectionIntent` 입력 경계가 구현됐다.
- Next production build의 정적 `/` 생성과 Playwright 1440×900·1280×800·1024×640 landscape 및 800×1280 portrait gate에서 6 passed·10 intended skips를 확인했다.
- Codex 인앱 브라우저 수동 점검에서 로비부터 Q-079·배신자 명령·결과·동일 입력 재판정·다음 턴이 동작했고 page console error는 0건이었다. Three.js 의존성의 `THREE.Clock` deprecation warning은 관찰 상태다.
- 위 결과는 로컬 desktop browser와 viewport emulation 범위다. 물리 Galaxy Tab S9+의 touch·S Pen·WebGL2, frame time·draw call·메모리·발열과 context 복구는 **NOT TESTED**다.

## 10. 확정된 표현 기본값과 남은 실측

다음 구현 기본값은 t0.2에서 확정했다.

- 근미래 정보기관 작전망의 절제된 dark neutral/navy 시각 언어
- 한국어 기본 UI와 Geist Sans 영문·숫자, Pretendard Variable 한글, Geist Mono 짧은 ASCII 메타데이터 역할 분리
- 북쪽 고정 3/4 perspective, 자유 회전 없음, 제한된 pan·zoom과 시점 초기화
- Galaxy Tab S9+ 가로 전용, 유효한 landscape viewport의 panel/drawer/bottom-sheet 재배치와 portrait orientation gate
- 48×48 CSS px 이상의 터치 대상과 색상·Canvas·hover에 의존하지 않는 DOM 동등 입력
- 실행 연출 건너뛰기와 reduced-motion 스냅

최종 3D 아트·제품명·로고·초상화·최종 사운드·최종 진영색은 token과 presentation adapter 뒤에 남겨 첫 수직 절편을 막지 않는다. 남은 필수 입력은 첫 실기기 통과 선언 전에 기록할 Galaxy Tab S9+의 정확한 모델 번호·Android·보안 패치·RAM·Chrome 전체 버전·CSS viewport·DPR·WebGL2 결과다. 이 값이 없어도 스캐폴드와 fixture 기반 UI 개발은 시작할 수 있다.
